...
# All normal generator settings live in circleGenConfig.toml instead of this
# source file. The config is loaded when this module is imported, which also
# means generator_adapter.py gets a fresh set of config values after reload().
...
def _load_config():
    """Load generator settings from the external TOML configuration."""
...
# ---------------- DERIVED VARIABLES ----------------
# SAFETY_DISTANCE intentionally remains derived from ORIGIN_Y so those two
# settings cannot get accidentally out of sync.
SAFETY_DISTANCE = ORIGIN_Y

# ---------------- CUTTING VARIABLES ----------------
# Values are loaded from circleGenConfig.toml above. Keep this section as a
# visual marker so the generator's computational code below stays readable.

# ---------------- MACHINE / FEEDER VARIABLES ----------------
# These values are also loaded from circleGenConfig.toml.

# ---------------- RARELY CHANGED ----------------
# These values are also loaded from circleGenConfig.toml.

# SAFETY_DISTANCE is derived from ORIGIN_Y above.
...
def emit_row(lines, row_y, segments, is_first_row=False):
    ...
    for i, seg in enumerate(segments):
        if seg[0] == "straight":
            ...
            elif CIRCLE_PRECUT_ANGLE > 0:
                r = d / 2
                cx = x_start + r  # circle center, plane-local (no ORIGIN offset)

                precut_angle_rad = math.radians(180.0 - CIRCLE_PRECUT_ANGLE)
                precut_x = cx + r * math.cos(precut_angle_rad)
                precut_y = row_y + r * math.sin(precut_angle_rad)

                lines.append(
                    f"G2 X{ORIGIN_X + precut_x:.3f} Y{ORIGIN_Y + precut_y:.3f} I{r:.3f} J0.000 "
                    f"F{FEED_RATE} ; first circle: pre-cut {CIRCLE_PRECUT_ANGLE:g} of "
                    f"upper half from the left point"
                )
                lines.append(
                    f"G3 X{ORIGIN_X + x_start:.3f} Y{abs_y:.3f} "
