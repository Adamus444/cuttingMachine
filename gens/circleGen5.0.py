...
SERIES-BASED CUTTING (per series, i.e. per small batch of circles)
  For each series (see split_into_series() / emit_series()):
    ...
    2. Move to the burn-off spot (SAFETY_BURN_OFF_X_DISTANCE,
       SAFETY_BURN_OFF_Y_DISTANCE below the row) and run the coating
       burn-off heat/cool cycle (see DUPLICATED BURN-OFF HEAT/COOL
       CYCLE below - unchanged), then reheat to CUTTING_TEMP and come
       back, ready to cut upper halves.
    ...
    4. Move to X0 and down to the burn-off spot again, run the same
       burn-off heat/cool cycle, but this time do NOT reheat - the wire
       is left cold, exactly like the end-of-row cleaning in
...
ARC OVERLAP (waste-severing detour return point)
  ...
  Now the non-cutting travel back lands ARC_OVERLAP_ANGLE degrees
  ...
  cut) instead of exactly at it. The arc is then resumed from there,
  all the way through to the circle's right point, in one G3 move. That
  move necessarily re-cuts that small ARC_OVERLAP_ANGLE-degree sliver
  of already-open kerf first (harmless - it's already cut) before
  going on to cut the genuinely new remaining material out to the right
  point - guaranteeing a clean, fully-severed join with no nub, at the
  cost of a few extra degrees of arc travel. ARC_OVERLAP_ANGLE must be
  strictly less than whichever arc-stop-angle applies to the row
  (ARC_STOP_ANGLE or ARC_STOP_ANGLE_FIRST_ROW), or the "3 degrees back"
  point would fall before the arc even started.
...
SERIES_SIZE = 2              # how many circles get cut lower-half -> burn off
...
def cut_lower_half(lines, row_y, x_start, x_end, d, row_arc_stop_angle, waste_cut_length):
    ...
    detour exactly as circleGenFeeder4_3.py did - but the non-cutting
    travel back after the detour lands ARC_OVERLAP_ANGLE degrees EARLIER
    along the arc than the original stop point, instead of exactly at
    it, and the arc is then resumed from there. See the ARC OVERLAP note
    ...
    # front-right (+X, -Y) quadrant, then non-cutting travel back - but
    # to the OVERLAP point below, not the exact stop point.
...
def emit_series(lines, row_y, abs_y, safety_y, batch, is_first_row, is_last_batch):
    ...
    last_upper_arc = None  # (x_start, x_end, r) of the LEFTMOST circle in
                            # this batch - used for the left-side rub-clean
    batch_start_x = None
    for i in range(len(batch) - 1, -1, -1):
        seg = batch[i]
        if seg[0] == "straight":
            x_start = ORIGIN_X + seg[1]
            lines.append(f"G1 X{x_start:.3f} Y{abs_y:.3f} F{FEED_RATE} ; travel back over margin/gap")
    ...
    lines.append(
        f"G1 X{batch_start_x:.3f} Y{safety_y:.3f} F{FEED_RATE} ; move to a safe spot, burn off coating"
    )
    burn_off_coating(lines)
    ...
    if GENERATE_POP_OUT:
        for seg in batch:
            if seg[0] == "straight":
                ...
                lines.append(f"G1 Y{abs_y:.3f} F{TRAVEL_RATE} ; move to pop out")
                lines.append(
