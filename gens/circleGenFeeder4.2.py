...
WASTE-SEVERING DIAGONAL CUT (bottom arc only)
  ...
  G3 pieces with a straight detour spliced in between them:
    1. G3 for the first ARC_STOP_ANGLE of the 180 (default 135, i.e.
       stopping 45 degrees short of the circle's rightmost point).
    2. A straight G1 cut, always at a fixed 45 degree angle toward the
       front-right (increasing X, decreasing Y) - no lookup of any other
       circle's position, just always the same direction. Its LENGTH
       depends on which row it's in: the very first row of the very
       first window (the true leading edge of fresh stock - same row
       SQUISH_ROOM-ON-Y singles out) uses WASTE_CUT_LENGTH_FIRST_ROW;
       every other row, in every window, uses WASTE_CUT_LENGTH.
...
ARC_STOP_ANGLE = 135.0  # degrees, stop the bottom 180° arc this far in,
...
WASTE_CUT_LENGTH_FIRST_ROW = 20.0  # mm, straight waste-severing detour length,
                             ...
                             # specially). Fixed 45° toward front-right;
                             # ignores other circles.

WASTE_CUT_LENGTH = 2.0     # mm, straight waste-severing detour length,
...
def emit_row(lines, row_y, segments, is_first_row=False):
    ...
    above: the wire (still cold, no reheat needed - everything here is
    either already-cut kerf or open air over an already fully separated
    piece) sweeps forward across the row one more time. Straight
    segments are just travelled over as usual. For every circle, it
    retraces the upper-half arc clockwise starting from the circle's
    left point, but only for the first POP_OUT_STOP_ANGLE degrees of
    that 180 degree sweep (already-cut kerf, so this is a retrace, not a
    new cut), then leaves the arc early and moves in a straight line
    (non-cutting) directly to the circle's right point. That straight
    jump crosses the open space where the piece used to sit, physically
    nudging the now fully-separated circle out of the way before the
    wire moves on to pop out the next one.
    """
    ...
    for i, seg in enumerate(segments):
        if seg[0] == "straight":
            ...
            # Bottom arc, part 1: the first ARC_STOP_ANGLE of the
            ...
            stop_angle_rad = math.radians(180.0 + ARC_STOP_ANGLE)
            ...
            lines.append(
                ...
                f"F{FEED_RATE} ; cut {ARC_STOP_ANGLE:g} of 180 deg of lower half, d={d:g}"
            )

            # Waste-severing detour: straight cut of fixed length (row-
            # dependent - see waste_cut_length above) at a fixed 45 degree
            # angle toward the front-right (+X, -Y) - not aimed at any
            ...
            # (180 - ARC_STOP_ANGLE) degrees, stop point -> right
            ...
            lines.append(
                ...
                f"; finish remaining {180.0 - ARC_STOP_ANGLE:g} of 180 deg of lower half"
            )
