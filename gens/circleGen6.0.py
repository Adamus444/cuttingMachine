...
CIRCLES_PER_CLEAN = 2       # how many circles the wire cuts, in EACH sweep,
...
def cut_lower_half(lines, row_y, x_start, x_end, d, row_arc_stop_angle, waste_cut_length,
    ...
    if do_periodic_clean:
        ...
        lines.append(
            ...
            f"; return from periodic clean, resume the arc at the original stop point"
        )
    else:
        # Plain path: straight back (non-cutting) to the exact point the
        # arc left off at, before resuming.
        lines.append(
            f"G1 X{ORIGIN_X + stop_x:.3f} Y{ORIGIN_Y + stop_y:.3f} F{TRAVEL_RATE} "
            f"; return from detour to resume the arc"
        )

    # Bottom arc, part 2: finish the remaining (180 - row_arc_stop_angle)
    # degrees, stop point -> right point. I/J are relative to THIS arc's
    # start point (the stop point), still pointing at the same untouched
    # circle center, so together with part 1 this still cuts a full,
    # perfect 180 degree bottom half - the detour (and the optional
    # periodic clean) is spliced in, not subtracted.
    lines.append(
