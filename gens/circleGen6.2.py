...
FIRST CIRCLE PRE-CUT (FIRST_CIRCLE_PRECUT_ANGLE)

  The very first circle segment of every row (main circle or margin
  filler, whichever comes first in that row) gets an extra pre-cut
  spliced in right before its normal lower-half cut: starting from its
  left point, the wire cuts FIRST_CIRCLE_PRECUT_ANGLE degrees of the
  UPPER half (up and over the top, at the default 90 degrees), then
  comes straight back down to the left point - then the lower half is
  cut completely normally (cut_lower_half(), same as any other circle),
  and later, in the reverse sweep, the full upper half is cut completely
  normally too (cut_upper_half()), just re-cutting this same small arc a
  second time in the process - harmless, already-open kerf. Set
  FIRST_CIRCLE_PRECUT_ANGLE = 0 to disable this entirely.
...
SEVER_DISTANCE = 20.0       # mm, extra cut to fully sever circles from waste.
...
FIRST_CIRCLE_PRECUT_ANGLE = 90.0
                             # degrees, the very first circle segment of
                             # every row (main circle or margin filler,
                             # whichever comes first) gets this much of
                             # its UPPER half pre-cut, starting from the
                             # left point, immediately BEFORE its normal
                             # lower-half cut - then the wire returns to
                             # the left point and the lower half (and
                             # later, in the reverse sweep, the full
                             # upper half again) cut exactly as normal.
                             # At 90 (the default) the pre-cut runs from
                             # the left point straight up to the circle's
                             # top point. Set to 0 to disable.
...
ARC_STOP_ANGLE = 135.0      # degrees, where the bottom 180 degree arc splits
                             ...
                             # horizontal.
WASTE_CUT_LENGTH = 15.0     # mm, straight waste-severing detour length,
                             # used for every other row (all rows after the
                             # first, in every window). Fixed 45 deg toward
                             # front-right; ignores other circles.
ARC_OVERLAP_ANGLE = 10.0     # degrees, how far EARLIER than the original arc
...
TRAVEL_RATE = 600            # mm/min, speed while travelling (X/Y, non-productive)
...
SAFETY_BURN_OFF_Y_DISTANCE = 50.0
...
def emit_row(lines, row_y, segments, is_first_row=False):
    ...
    for i, seg in enumerate(segments):
        if seg[0] == "straight":
            ...
            if i == first_circle_index and FIRST_CIRCLE_PRECUT_ANGLE > 0:
                # First circle of the row: before the normal lower-half
                # cut below, pre-cut FIRST_CIRCLE_PRECUT_ANGLE degrees of
                # the UPPER half starting from the left point (up over
                # the top), then come straight back down to the left
                # point. This does NOT replace or skip anything - the
                # lower half still gets cut normally right below, and
                # later, in the reverse sweep, the upper half still gets
                # cut normally too (it'll just be re-cutting this same
                # arc a second time then - harmless, already-open kerf).
                r = d / 2
                cx = x_start + r  # circle center, plane-local (no ORIGIN offset)

                # I/J below are always relative to the arc's OWN start
                # point, not the circle center - same convention used
                # throughout cut_lower_half()/cut_upper_half(). At the
                # default 90 degrees this lands exactly on the circle's
                # top point; other angles land elsewhere on the upper arc.
                precut_angle_rad = math.radians(180.0 - FIRST_CIRCLE_PRECUT_ANGLE)
                ...
                lines.append(
                    ...
                    f"F{FEED_RATE} ; first circle: pre-cut {FIRST_CIRCLE_PRECUT_ANGLE:g} of "
                    f"upper half from the left point"
                )
                lines.append(
                    f"G3 X{ORIGIN_X + x_start:.3f} Y{abs_y:.3f} "
                    f"I{cx - precut_x:.3f} J{row_y - precut_y:.3f} F{TRAVEL_RATE} "
                    f"; first circle: return from pre-cut back to the left point"
                )
...
def generate_gcode():
    ...
    if not (0.0 <= FIRST_CIRCLE_PRECUT_ANGLE < 180.0):
        raise ValueError(
            "FIRST_CIRCLE_PRECUT_ANGLE must be 0 (disabled) or more, and "
            "strictly less than 180 degrees - it's a piece of the upper "
            "180 degree arc, so 180 would try to pre-cut the entire thing."
        )
