...
WASTE-SEVERING DIAGONAL CUT (bottom arc only)
  ...
  G3 pieces with a straight detour spliced in between them. Which row
  we're in picks ONE arc-stop-angle up front, row_arc_stop_angle - the
  very first row of the very first window (the true leading edge of
  fresh stock - same row SQUISH_ROOM-ON-Y singles out) uses
  ARC_STOP_ANGLE_FIRST_ROW (default 90); every other row, in every
  window, uses ARC_STOP_ANGLE (default 135). That single value then
  drives every part of the detour below - the split point and the
  detour's direction always agree, because they come from the same
  number:
    1. G3 for the first row_arc_stop_angle of the 180 (default 90 for
       the first row - stopping right at the bottom of the arc; 135 for
       every other row - stopping 45 degrees short of the circle's
       rightmost point). Neither value is special-cased beyond that -
       any value strictly between 0 and 180 works for either one; the
       split point is just wherever row_arc_stop_angle says it is.
    2. A straight G1 cut whose direction tracks row_arc_stop_angle:
       measured below horizontal, the detour angle is
       (180 - row_arc_stop_angle) degrees, always toward the front-right
       quadrant (increasing X, decreasing Y) - no lookup of any other
       circle's position, just this one formula. At the first row's
       default of 90 that works out to 90 degrees, i.e. straight down;
       at every other row's default of 135 that's a 45 degree line (135
       stops 45 degrees short of the rightmost point, and the waste
       there sits at 45 degrees off horizontal). Its LENGTH also depends
       on which row it's in: WASTE_CUT_LENGTH_FIRST_ROW for the first
       row of the first window, WASTE_CUT_LENGTH for every other row.
    ...
    4. A second G3, continuing from that same point through the
       remaining (180 - ARC_STOP_ANGLE) degrees to the circle's
...
FIRST CIRCLE OF EVERY ROW (cut immediately, both halves)
  Every other circle in a row gets its lower half cut on the forward
  sweep and its upper half cut later, on the reverse sweep (see emit_row
  below). The very FIRST circle segment encountered in each row (the
  first "circle" entry in that row's segment list, whatever it is -
  main circle or a margin filler) can't wait that long: for physical
  reasons it has to come out fully cut before the wire ever moves on to
  the rest of the row. So for that one circle only:
    1. Cut its lower half exactly as normal (with the waste-severing
       detour spliced in, same as any other circle).
    2. Without touching the temperature at all (no M101, wire stays at
       cutting temp), travel back (non-cutting) to that circle's own
       left point.
    3. Cut its upper half right there (mirrored direction of the normal
       reverse-sweep cut - G2 instead of G3, left point -> right point -
       plus the same sever-cut-and-return used elsewhere), landing back
       at the circle's right point.
    4. Resume the forward sweep exactly where it left off, cutting lower
       halves of the rest of the row as always.
  Because that first circle is already fully cut, the later reverse
  sweep skips cutting its upper half again - it just travels across it
  (like an already-open gap) instead, though it still counts as the
  "last upper arc" for the left-side wire-cleaning rub (same kerf,
  doesn't matter which direction it was originally cut in). Every other
  circle in the row is completely unaffected by this and still gets cut
  the same way it always has.

POP-OUT PASS TOGGLE
  GENERATE_POP_OUT controls whether the pop-out pass described on
  emit_row runs at all. Some setups don't want the machine nudging cut
  circles out on its own (e.g. picking them out by hand instead) - set
  GENERATE_POP_OUT = False to skip generating that pass completely; the
  row then just ends after the left-side wire-cleaning rub.

DUPLICATED BURN-OFF HEAT/COOL CYCLE
  Both of the burn-off-coating spots in emit_row (the right-side clean
  after the forward sweep, and the end-of-row clean after the reverse
  sweep) used to run the cycle once: heat to BURN_OFF_TEMP, hold
  BURN_OFF_TIME, cool to 0, hold TEMP_SET_TIME. That's now run twice
  back to back before the wire moves anywhere else - heat, cool, heat,
  cool, then continue - with nothing else (no G1) spliced in between,
  since the wire is meant to sit still through the whole thing.
...
WASTE_CUT_LENGTH = 4.0     # mm, straight waste-severing detour length,
                             ...
                             # first, in every window). Fixed 45° toward
...
def emit_row(lines, row_y, segments, is_first_row=False):
    ...
    is_first_row selects which waste-severing detour length is used for
    every circle in this row: WASTE_CUT_LENGTH_FIRST_ROW when True (meant
    for only the very first row of the very first window - the true
    leading edge of fresh stock), WASTE_CUT_LENGTH otherwise (every other
    row, in every window). See the WASTE-SEVERING DIAGONAL CUT note in
    the module docstring.
    ...
    Pop-out pass, once per row, right after the left-side cleaning
    above, ONLY when GENERATE_POP_OUT is True (otherwise the row simply
    ends after that left-side cleaning): the wire (still cold, no reheat
    needed - everything here is either already-cut kerf or open air over
    an already fully separated piece) sweeps forward across the row one
    more time. Straight segments are just travelled over as usual. For
    every circle, it retraces the upper-half arc clockwise starting from
    the circle's left point, but only for the first POP_OUT_STOP_ANGLE
    degrees of that 180 degree sweep (already-cut kerf, so this is a
    retrace, not a new cut), then leaves the arc early and moves in a
    straight line (non-cutting) directly to the circle's right point.
    That straight jump crosses the open space where the piece used to
    sit, physically nudging the now fully-separated circle out of the
    way before the wire moves on to pop out the next one.

    First circle of the row: the first "circle" segment encountered gets
    both halves cut immediately, back to back, instead of waiting for
    the reverse sweep - see the FIRST CIRCLE OF EVERY ROW note in the
    module docstring. The reverse sweep then just travels across that
    same circle instead of re-cutting it.
    """
    ...
    # The first circle segment in this row - main circle or filler, whichever
    # comes first - gets both halves cut immediately instead of waiting for
    # the reverse sweep. See the FIRST CIRCLE OF EVERY ROW note in the
    # module docstring.
    first_circle_index = next(
        (i for i, seg in enumerate(segments) if seg[0] == "circle"), None
    )
    ...
    for i, seg in enumerate(segments):
        if seg[0] == "straight":
            ...
            if i == segments_max_index:
                ...
                # Wire sits still through the whole cycle: heat, cool, heat,
                # cool again, then move - see the DUPLICATED BURN-OFF
                # HEAT/COOL CYCLE note in the module docstring.
            ...
            # Which arc-stop-angle applies to THIS row: the first row of the
            # first window uses ARC_STOP_ANGLE_FIRST_ROW, every other row
            # uses ARC_STOP_ANGLE. This single value drives BOTH where the
            # arc actually splits and which direction the detour runs -
            # those two things must always agree, or the detour ends up
            # pointing somewhere other than where the arc really stopped.
            row_arc_stop_angle = ARC_STOP_ANGLE_FIRST_ROW if is_first_row else ARC_STOP_ANGLE

            # Bottom arc, part 1: the first row_arc_stop_angle of the
            ...
            # Waste-severing detour: straight cut of fixed length (row-
            # dependent - see waste_cut_length above), toward the
            # front-right (+X, -Y) quadrant, at an angle that tracks
            # row_arc_stop_angle: measured below horizontal, the detour
            # angle is (180 - row_arc_stop_angle) degrees - 45 degrees at
            # ARC_STOP_ANGLE=135, straight down (90 degrees) at
            # ARC_STOP_ANGLE_FIRST_ROW=90, and so on. Not aimed at any
            # other circle - then travel straight back (non-cutting) to
            # the exact point the arc left off at.
            ...
            # (180 - row_arc_stop_angle) degrees, stop point -> right
            ...
            lines.append(
                ...
                f"; finish remaining {180.0 - row_arc_stop_angle:g} of 180 deg of lower half"
            )
    ...
    for i in range(len(segments) - 1, -1, -1):
        ...
        if seg[0] == "straight":
            ...
            # Whether it was just cut here or already cut during the forward
            # sweep, this is still valid already-cut kerf to rub the wire
            # clean on - see last_upper_arc's use below.
    ...
    # Wire sits still through the whole cycle: heat, cool, heat, cool again,
    # then move - see the DUPLICATED BURN-OFF HEAT/COOL CYCLE note in the
    # module docstring.
    lines.append(f"M101 P{COOL_DOWN_TIME} R0 ; cool down the wire")
    lines.append(f"M101 P{BURN_OFF_TIME} R{BURN_OFF_TEMP} ; heat up, burn off coating")
    lines.append(f"M101 P{COOL_DOWN_TIME} R0 ; cool down the wire")
    ...
    # clockwise from its left point for only the first
    # POP_OUT_STOP_ANGLE degrees of the 180 degree sweep (already-cut
    # kerf), then the wire leaves the arc early and jumps in a straight,
    # non-cutting line to the circle's right point - crossing the open
    # space where the piece used to sit, nudging it out of the way before
    # moving on to pop out the next one. See the docstring above.
    #
    # GENERATE_POP_OUT = False skips this pass entirely - see the
    # POP-OUT PASS TOGGLE note in the module docstring.
    if GENERATE_POP_OUT:
        for seg in segments:
            if seg[0] == "straight":
                x_end = ORIGIN_X + seg[2]
                ...
                # Stop point of the retrace: starting at the left point
                # (angle 180 deg) and sweeping clockwise (decreasing angle,
                # through the top) for POP_OUT_STOP_ANGLE degrees.
                ...
                # Retrace the upper arc clockwise (G2) from the left point.
                # I/J are relative to this arc's start point (x_start, row_y),
                # pointing at the untouched circle center - mirrors the G3
                # used to originally cut this same upper half in the reverse
                # sweep above, just the opposite direction and cut short.
                lines.append(
                ...
                # Leave the arc early and jump straight (non-cutting) to the
                # circle's right point - the piece is already fully
                # separated, so this just pushes it out of the way.
                lines.append(
...
def generate_gcode():
    ...
    while True:
        ...
        for row_index, (row_y, segments) in enumerate(window):
            # Only the very first row of the very first window (the true
            # leading edge of fresh stock) uses WASTE_CUT_LENGTH_FIRST_ROW -
            # see the is_first_row note on emit_row().
