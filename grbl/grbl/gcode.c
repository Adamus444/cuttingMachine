...
#include <util/delay.h> // dirty solution for pushing the cut circles
...
uint8_t gc_execute_line(char *line)
{
        ...
        switch(int_value) {
          ...
          case 3: case 4: case 5:
            ...
            case 56:
              ...
              break;
          #endif
          case USER_MCODE_TEMP:   // MANUAL ADDITION
            word_bit = MODAL_GROUP_M10;              // pick next unused number in gcode.h
            gc_block.user_mcode = USER_MCODE_TEMP;   // <-- new field you add, so G4 alone doesn't trigger this
            break;
          case USER_MCODE_KNOCK: // MANUAL ADDITION
          printPgmString(PSTR("[MSG:knock initiated]\r\n"));
            {
                DDRC |= (1<<5);   // A5 = PC5 on Uno, set as output (safe to call every time)
            
                uint16_t steps = 400; // steps for one full rev — depends on TB6600 microstep DIP setting
                                       // e.g. 200 = full step (1.8°/step), 400 = half step, 1600 = 1/8, etc.
            
                for (uint16_t i = 0; i < steps; i++) {
                    PORTC |= (1<<5);
                    _delay_us(500);      // pulse high time — tune for speed/torque
                    PORTC &= ~(1<<5);
                    _delay_us(500);      // pulse low time
                }
            }
            break;
  ...
  */
  ...
  // MANUAL ADDITION
  if (gc_block.user_mcode == USER_MCODE_TEMP) {
    if (bit_isfalse(value_words,bit(WORD_P))) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } // [P word missing]
    bit_false(value_words,bit(WORD_P));
  }

  if (gc_block.user_mcode == USER_MCODE_TEMP) {
    if (bit_isfalse(value_words,bit(WORD_R))) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); }
    if (gc_block.values.r < 0.0 || gc_block.values.r > 100.0) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } // or nearest matching status code
  ...
  */
  ...
  // [10. Dwell ]:
  if (gc_block.user_mcode == USER_MCODE_TEMP) {  // MANUAL ADDITION
    protocol_buffer_synchronize();       // wait for any queued motion to finish first
    temp_control_set(gc_block.values.r);   // your closed-loop knob-turn routine, blocking
